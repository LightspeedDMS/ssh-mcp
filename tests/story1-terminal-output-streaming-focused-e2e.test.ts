...
describe('Story 1: Terminal Output Streaming - Focused E2E', () => {
  ...
  test('should capture real-time terminal output with listeners and maintain history', async () => {
    ...
    // Execute command
    const result = await sshManager.executeCommand(sessionName, 'echo "Real-time test"');
    ...
    // Verify command prompt captured
  ...
  test('should maintain session isolation and format detection', async () => {
    ...
    const formattedEntries1 = history1.filter(entry => entry.preserveFormatting);
  ...
  test('should meet performance requirements (latency and buffer management)', async () => {
    ...
    const outputListener = (outputEntry: any) => {
      if (outputEntry.output.includes('Latency test') && outputReceivedTime === 0) {
    ...
    console.log(`✅ Memory usage: ~${estimatedMemoryMB.toFixed(2)}MB for ${terminalHistory.length} entries`);
