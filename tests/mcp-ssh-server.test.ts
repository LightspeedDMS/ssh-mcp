...
describe("MCPSSHServer", () => {
  ...
  describe("MCP Server Functionality", () => {
    ...
    it("should handle ssh_connect tool call", async () => {
      ...
      expect((result as any).connection.name).toBe("test-session");
    });
    ...
    it("should handle ssh_exec tool call", async () => {
      ...
      expect((result as any).success).toBe(true);
      expect((result as any).result.stdout).toBe("hello world");
    });

    it("should NOT have HTTP server capabilities", () => {
      expect(() => (mcpServer as any).getWebPort()).toThrow(
        "HTTP functionality not available",
      );
      expect(() => (mcpServer as any).isWebServerRunning()).toThrow(
        "HTTP functionality not available",
      );
    });
    ...
    it("should coordinate with web server for monitoring URLs", async () => {
      ...
      expect((result as any).success).toBe(true);
      expect((result as any).monitoringUrl).toBe(
        "http://localhost:8080/session/test-session",
      );
    });
    ...
    it("should fail monitoring URL when web server port not set", async () => {
      ...
      expect((result as any).error).toContain("Web server not available");
    });
  });
