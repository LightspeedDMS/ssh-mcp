...
describe('Story 4: MCP Tool Integration - Simplified E2E', () => {
  ...
  describe('MCP Tool Availability and Integration', () => {
    ...
    test('should generate monitoring URLs and trigger web server start', async () => {
      ...
      if (urlData.success) {
        ...
        console.log('✅ URL generation fails appropriately for non-existent session');
      }
    });
    ...
    test('should support session lifecycle operations', async () => {
      ...
      if (connectionCreated) {
        expect(finalData.sessions.length).toBeGreaterThan(initialData.sessions.length);
        const newSession = finalData.sessions.find((s: any) => s.name === sessionName);
  ...
  describe('Terminal Streaming Infrastructure Validation', () => {
    test('should support session-specific WebSocket endpoint format', async () => {
      ...
      const expectedUrlPattern = new RegExp(`http://localhost:${testPort}/session/${sessionName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}`);
      ...
      if (!urlResult.isError) {
        ...
        if (urlData.success) {
          ...
          const wsUrl = urlData.monitoringUrl.replace('http://', 'ws://').replace(`/session/${sessionName}`, `/ws/session/${sessionName}`);
          expect(wsUrl).toContain(`ws://localhost:${testPort}/ws/session/${sessionName}`);
        }
      }
    ...
    test('should maintain proper error propagation for terminal streaming', async () => {
      ...
      for (const testCase of testCases) {
        try {
          ...
          expect((error as Error).message.toLowerCase()).toContain(testCase.expectedError);
        }
      }
  ...
  describe('Integration Requirements Validation', () => {
    test('should support all MCP tool activity integration points', async () => {
      ...
      const connectSchema = (await client.listTools()).tools.find(t => t.name === 'ssh_connect');
      expect(connectSchema?.description).toContain('connection');
      
      // Command execution integration  
      const execSchema = (await client.listTools()).tools.find(t => t.name === 'ssh_exec');
      expect(execSchema?.description).toContain('command');
      
      // Disconnection integration
      const disconnectSchema = (await client.listTools()).tools.find(t => t.name === 'ssh_disconnect');
      expect(disconnectSchema?.description).toContain('disconnect');
      
      // Monitoring integration
      const monitorSchema = (await client.listTools()).tools.find(t => t.name === 'ssh_get_monitoring_url');
    ...
    test('should handle concurrent session operations', async () => {
      ...
      const operations = [
        client.callTool({
          name: 'ssh_connect',
          arguments: { name: session1, host: 'localhost', username: 'test_user', password: 'password123' }
        }),
        client.callTool({
          name: 'ssh_connect', 
          arguments: { name: session2, host: 'localhost', username: 'test_user', password: 'password123' }
        })
      ];
      ...
      // Cleanup
      try {
        await client.callTool({ name: 'ssh_disconnect', arguments: { sessionName: session1 } });
        await client.callTool({ name: 'ssh_disconnect', arguments: { sessionName: session2 } });
      } catch (error) {
