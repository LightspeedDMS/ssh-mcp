...
describe("SSH Command Echo Unit Tests", () => {
  ...
  describe("Command Echo Generation - Zero History", () => {
    it("should generate exact WebSocket output for zero history (just initial prompt)", async () => {
      ...
      // Execute command that should generate echo
      await connectionManager.executeCommand("test-session", "whoami", { source: "user" });

      // EXACT assertion - match the entire WebSocket output string
      const expectedOutput = "[jsbattig@localhost ~]$ whoami\r\njsbattig";
      expect(capturedWebSocketOutput).toBe(expectedOutput);
    });
  });

  describe("Command Echo Generation - One Command History", () => {
    it("should generate exact WebSocket output for single command in history", async () => {
      // Mock successful SSH connection
      mockClient.on.mockImplementation((event: string, callback: (...args: any[]) => void) => {
        if (event === 'ready') {
          setTimeout(() => callback(), 10);
      ...
      // Mock pwd command for directory discovery
      const pwdStream = new MockStream();
      mockClient.exec
        .mockImplementationOnce((command: string, callback: (err: Error | undefined, stream: any) => void) => {
          if (command === 'pwd') {
            setTimeout(() => callback(undefined, pwdStream), 10);
            pwdStream.simulateCommand('/home/jsbattig');
            return mockClient;
          }
        })
        .mockImplementationOnce((command: string, callback: (err: Error | undefined, stream: any) => void) => {
          if (command === 'pwd') {
            const stream = new MockStream();
            setTimeout(() => callback(undefined, stream), 10);
            stream.simulateCommand('/home/jsbattig');
            return mockClient;
          }
        })
        .mockImplementationOnce((command: string, callback: (err: Error | undefined, stream: any) => void) => {
          if (command === 'whoami') {
            const stream = new MockStream();
            setTimeout(() => callback(undefined, stream), 10);
            stream.simulateCommand('jsbattig');
            return mockClient;
          }
        });

      // Execute first command to establish history
      await connectionManager.executeCommand("test-session", "pwd", { source: "user" });

      // Clear captured output for second command test
      capturedWebSocketOutput = "";
  ...
  describe("Command Echo Generation - Multiple Commands History", () => {
    it("should generate exact WebSocket output for multiple commands in history", async () => {
      // Mock successful SSH connection
      mockClient.on.mockImplementation((event: string, callback: (...args: any[]) => void) => {
        if (event === 'ready') {
          setTimeout(() => callback(), 10);
      ...
      // Set up mock for multiple commands
      let callCount = 0;
      mockClient.exec.mockImplementation((command: string, callback: (err: Error | undefined, stream: any) => void) => {
        callCount++;
        const stream = new MockStream();
        ...
        if (command === 'pwd') {
          ...
          stream.simulateCommand('Wed Jan 15 10:30:45 PST 2025');
        }
        return mockClient;
      });

      // Execute first two commands to build history
      await connectionManager.executeCommand("test-session", "pwd", { source: "user" });
      await connectionManager.executeCommand("test-session", "whoami", { source: "user" });

      // Clear captured output for third command test
      capturedWebSocketOutput = "";

      // Execute third command
      await connectionManager.executeCommand("test-session", "date", { source: "user" });

      // EXACT assertion - match the entire WebSocket output string
      const expectedOutput = "[jsbattig@localhost ~]$ date\r\nWed Jan 15 10:30:45 PST 2025";
      expect(capturedWebSocketOutput).toBe(expectedOutput);
    });
  });

  describe("Command Echo Generation - Multi-column Output", () => {
    it("should generate exact WebSocket output for ls -la multi-column output", async () => {
      // Mock successful SSH connection
      mockClient.on.mockImplementation((event: string, callback: (...args: any[]) => void) => {
        if (event === 'ready') {
          setTimeout(() => callback(), 10);
...
drwx------  2 jsbattig jsbattig  4096 Jan 15 10:30 .ssh`;
          stream.simulateCommand(lsOutput);
        }
        return mockClient;
      });

      // Execute pwd to establish directory cache
      await connectionManager.executeCommand("test-session", "pwd", { source: "system" });
...
drwx------  2 jsbattig jsbattig  4096 Jan 15 10:30 .ssh`;
  ...
  describe("CRLF Formatting - xterm Compatibility", () => {
  ...
  describe("Source-based Command Echo Behavior", () => {
    it("should generate command echo for user source commands", async () => {
      ...
      mockClient.exec.mockImplementation((command: string, callback: (err: Error | undefined, stream: any) => void) => {
        ...
        if (command === 'pwd') {
          ...
          stream.simulateCommand('uid=1000(jsbattig) gid=1000(jsbattig) groups=1000(jsbattig)');
        }
        return mockClient;
      });
    ...
    it("should generate command echo for claude source commands", async () => {
      ...
      mockClient.exec.mockImplementation((command: string, callback: (err: Error | undefined, stream: any) => void) => {
        ...
        if (command === 'pwd') {
          ...
          stream.simulateCommand('localhost');
        }
        return mockClient;
      });
    ...
    it("should NOT generate command echo for system source commands", async () => {
      ...
      mockClient.exec.mockImplementation((command: string, callback: (err: Error | undefined, stream: any) => void) => {
        ...
        if (command === 'uptime') {
          stream.simulateCommand('10:30:45 up 1 day,  2:15,  1 user,  load average: 0.15, 0.10, 0.08');
        }
        return mockClient;
      });
