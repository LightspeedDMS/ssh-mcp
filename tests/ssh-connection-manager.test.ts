...
describe("SSHConnectionManager", () => {
  ...
  describe("SSH Connection Establishment", () => {
    ...
    it("should maintain connection metadata (host, user, last activity)", async () => {
      ...
      expect(connection.lastActivity.getTime()).toBeGreaterThanOrEqual(
        beforeConnection,
      );
      expect(connection.lastActivity.getTime()).toBeLessThanOrEqual(
        afterConnection,
      );
    }, 15000);
  ...
  describe("Error Handling", () => {
    it("should handle connection errors with invalid credentials", async () => {
      ...
      await expect(
        connectionManager.createConnection(invalidConfig),
      ).rejects.toThrow();
    }, 15000);
    ...
    it("should handle connection timeout", async () => {
      ...
      await expect(
        connectionManager.createConnection(timeoutConfig),
