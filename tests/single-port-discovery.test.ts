...
describe('Single Port Discovery Mechanism', () => {
  ...
  afterEach(async () => {
    ...
    await Promise.all(occupiedServers.map(server => 
  ...
  describe('findAvailablePort', () => {
    ...
    it('should handle port range exhaustion', async () => {
      ...
      await expect(findAvailablePort(65535))
        .rejects.toThrow('No available ports found in range');
    });
  });
  ...
  describe('PortManager', () => {
    ...
    it('should prevent double reservation of same port', async () => {
      ...
      await expect(portManager.reservePortDirectly(testPort))
        .rejects.toThrow('Port already reserved');
    });
    ...
    it('should validate port range', async () => {
      await expect(portManager.reservePortDirectly(0))
        .rejects.toThrow('Invalid port');
      
      await expect(portManager.reservePortDirectly(70000))
        .rejects.toThrow('Invalid port');
    });
