...
describe("SSHConnectionManager - Multiple Session Management (Story 5)", () => {
  ...
  describe("Session Isolation - Working Directories", () => {
    it("should maintain isolated working directories for each session", async () => {
      ...
      const session1Pwd = await connectionManager.executeCommand(
        "session-1",
        "pwd",
      );
      ...
      const session2Pwd = await connectionManager.executeCommand(
        "session-2",
        "pwd",
      );
  ...
  describe("Session Isolation - Environment Variables", () => {
    it("should maintain isolated environment variables for each session", async () => {
      ...
      await connectionManager.executeCommand(
        "session-env-1",
        'export TEST_VAR="session1_value"',
      );
      await connectionManager.executeCommand(
        "session-env-2",
        'export TEST_VAR="session2_value"',
      );

      // Check environment variable isolation
      const session1Env = await connectionManager.executeCommand(
        "session-env-1",
        "echo $TEST_VAR",
      );
      const session2Env = await connectionManager.executeCommand(
        "session-env-2",
        "echo $TEST_VAR",
      );
  ...
  describe("Session Name Uniqueness and Validation", () => {
    it("should ensure session names are unique", async () => {
      ...
      await expect(
        connectionManager.createConnection(sessionConfig),
  ...
  describe("Session Disconnection by Name", () => {
    it("should disconnect specific session by name", async () => {
      ...
      const connection1 = connectionManager.getConnection(
        "disconnect-session-1",
      );
      const connection2 = connectionManager.getConnection(
    ...
    it("should throw error when disconnecting non-existent session", async () => {
      await expect(
        connectionManager.disconnectSession("non-existent-session"),
  ...
  describe("Performance Requirements", () => {
    ...
    it("should prevent cross-session interference under concurrent load", async () => {
      ...
      const commandPromises = [
        connectionManager.executeCommand(
          "isolation-1",
          'export ISOLATION_TEST="value1" && echo $ISOLATION_TEST',
        ),
        connectionManager.executeCommand(
          "isolation-2",
          'export ISOLATION_TEST="value2" && echo $ISOLATION_TEST',
        ),
        connectionManager.executeCommand(
          "isolation-3",
          'export ISOLATION_TEST="value3" && echo $ISOLATION_TEST',
        ),
      ];
