...
export class MCPSSHServer {
  ...
  constructor(config: MCPSSHServerConfig = {}) {
    ...
    // SSH manager initialization - port will be set later by orchestrator
    this.sshManager = new SSHConnectionManager(8080); // Default port, will be updated
  ...
  async start(): Promise<void> {
    try {
      ...
      throw new Error(`Failed to start MCP SSH server: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async stop(): Promise<void> {
    try {
      ...
      if (this.config.logLevel !== 'silent') {
        console.error('Error during MCP SSH server cleanup:', error instanceof Error ? error.message : String(error));
      }
    }
  }
