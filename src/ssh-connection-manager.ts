...
import { SSHConnection, SSHConnectionConfig, ConnectionStatus, CommandResult, CommandOptions, CommandHistoryEntry } from './types.js';

export interface ISSHConnectionManager {
  getTerminalHistory(sessionName: string): TerminalOutputEntry[];
  addTerminalOutputListener(sessionName: string, callback: (entry: TerminalOutputEntry) => void): void;
  removeTerminalOutputListener(sessionName: string, callback: (entry: TerminalOutputEntry) => void): void;
  ...
  addCommandHistoryListener(sessionName: string, callback: (entry: CommandHistoryEntry) => void): void;
  removeCommandHistoryListener(sessionName: string, callback: (entry: CommandHistoryEntry) => void): void;
}
...
export class SSHConnectionManager implements ISSHConnectionManager {
  ...
  private broadcastToLiveListeners(sessionName: string, data: string, stream: 'stdout' | 'stderr' = 'stdout'): void {
    const sessionData = this.connections.get(sessionName);
    ...
    sessionData.outputListeners.forEach(listener => {
      try {
        ...
        console.warn(`Failed to notify terminal listener for session ${sessionName}:`, error);
      }
    });
  }

  // Store complete terminal interaction in history for new connections
  private storeInHistory(sessionName: string, data: string, stream: 'stdout' | 'stderr' = 'stdout'): void {
    const sessionData = this.connections.get(sessionName);
    if (!sessionData) return;
  ...
  private broadcastTerminalOutput(sessionName: string, data: string, stream: 'stdout' | 'stderr' = 'stdout'): void {
  ...
  private async initializeShellSession(sessionData: SessionData): Promise<void> {
    return new Promise((resolve, reject) => {
  ...
  async executeCommand(connectionName: string, command: string, options: CommandOptions = {}): Promise<CommandResult> {
    ...
    if (!sessionData.isShellReady || !sessionData.shellChannel) {
      throw new Error(`Shell session not ready for connection '${connectionName}'. Cannot maintain session state without active shell.`);
    }
  ...
  private executeCommandInShell(
    ...
    if (trimmedCommand === 'exit' || trimmedCommand.startsWith('exit ')) {
      commandEntry.reject(new Error(`Command '${trimmedCommand}' would terminate the shell session`));
      return;
    }
    ...
    const timeoutHandle = setTimeout(() => {
      if (sessionData.currentCommand && !hasPrompt && sessionData.shellChannel) {
        sessionData.shellChannel.removeListener('data', onData);
        commandEntry.reject(new Error(`Command '${commandEntry.command}' timed out after ${timeoutMs}ms`));
  ...
  private completeSimpleCommand(sessionData: SessionData, onData: (data: Buffer) => void, rawOutput: string): void {
    ...
    // Clean output by removing shell control sequences and prompts
    ...
    for (const line of lines) {
      ...
      // Skip exit code extraction since we're not capturing it anymore
      ...
      trimmed = trimmed.replace(/^[a-zA-Z0-9_.-]+@[a-zA-Z0-9_.-]+\s*[~\w/.[\]-]*[$#>]\$?\s*/, '');
      
      // Remove prompt suffix if it appears at the end of actual output  
      trimmed = trimmed.replace(/\s*[\]a-zA-Z0-9_@.[[#>-]+\s+[~\w/.-]*[\]$#>]\s*$/, '');
    ...
    // Send incremental updates to live connected browsers
    const prompt = `[${sessionData.connection.username}@${sessionData.connection.host} ~]$ `;
    
    // 1. Broadcast command with prompt (live update)
    this.broadcastToLiveListeners(sessionData.connection.name, `${prompt}${sessionData.currentCommand.command}\n`);
    
    // 2. Broadcast command output (live update)
    if (commandOutput) {
      this.broadcastToLiveListeners(sessionData.connection.name, commandOutput + '\n');
    }
    
    // 3. Broadcast new prompt (live update)
    this.broadcastToLiveListeners(sessionData.connection.name, prompt);
    
    // Store complete interaction as single history entry for new connections
    let completeOutput = `${prompt}${sessionData.currentCommand.command}\n`;
    if (commandOutput) {
      completeOutput += commandOutput + '\n';
    }
    completeOutput += prompt;
    
    this.storeInHistory(sessionData.connection.name, completeOutput);
  ...
  async disconnectSession(name: string): Promise<void> {
    ...
    this.broadcastTerminalOutput(name, `Connection to ${sessionData.connection.host} closed`);
  ...
  // Terminal output streaming methods
  private streamTerminalOutput(sessionData: SessionData, output: string, stream: 'stdout' | 'stderr'): void {
  ...
  sendTerminalInput(sessionName: string, input: string): void {
    ...
    if (!sessionData.isShellReady || !sessionData.shellChannel) {
      throw new Error(`Shell session not ready for connection '${sessionName}'`);
    }
  ...
  resizeTerminal(sessionName: string, cols: number, rows: number): void {
    ...
    if (cols < 1 || cols > 1000 || rows < 1 || rows > 1000) {
      throw new Error('Invalid terminal dimensions: cols and rows must be between 1 and 1000');
    }
  ...
  removeCommandHistoryListener(sessionName: string, callback: (entry: CommandHistoryEntry) => void): void {
    ...
    sessionData.commandHistoryListeners = sessionData.commandHistoryListeners.filter(
      listener => listener.callback !== callback
    );
  }
